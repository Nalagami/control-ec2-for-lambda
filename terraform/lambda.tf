...
resource "null_resource" "deploy_lambda_function" {
  ...
  #   triggers = {
  #     "code_diff" = filebase64("${path.module}/lambda/")
  #   }

  # ディレクトリ階層を無視(-j)して関数コードをzipアーカイブする
  provisioner "local-exec" {
    command = "zip -j ${path.module}/lambda_function.py.zip ${path.module}/lambda/*"
  }
...
resource "aws_iam_policy" "ec2_execution_policy" {
  name        = "ec2-execution-policy"
  description = "Policy for EC2 execution from Lambda"
...
# resource "aws_lambda_permission" "allow_start_lambda" {
#   action        = "lambda:InvokeFunction"
#   function_name = aws_lambda_function.slashcommand_minecraft_ec2_start.function_name
#   principal     = "lambda.amazonaws.com"
# }
# resource "aws_lambda_permission" "allow_stop_lambda" {
#   action        = "lambda:InvokeFunction"
#   function_name = aws_lambda_function.slashcommand_minecraft_ec2_stop.function_name
#   principal     = "lambda.amazonaws.com"
# }
# resource "aws_lambda_permission" "allow_status_lambda" {
#   action        = "lambda:InvokeFunction"
#   function_name = aws_lambda_function.slashcommand_minecraft_ec2_status.function_name
#   principal     = "lambda.amazonaws.com"
# }
...
resource "aws_lambda_function" "slashcommand_app" {
  function_name = "slashcommand-app"
  description   = ""
...
resource "aws_lambda_function" "slashcommand_minecraft_ec2_start" {
  function_name = "slashcommand-minecraft-ec2-start"
  description   = ""
...
resource "aws_lambda_function" "slashcommand_minecraft_ec2_stop" {
  function_name = "slashcommand-minecraft-ec2-stop"
  description   = ""
...
resource "aws_lambda_function" "slashcommand_minecraft_ec2_status" {
  function_name = "slashcommand-minecraft-ec2-status"
  description   = ""
...
resource "aws_lambda_function_url" "endpoint" {
  function_name      = aws_lambda_function.slashcommand_app.function_name
